...
hprismeval<-function (y, wt,parms)
{
  ...
  list(label = c(mean(y[, 1]), tfit.coef[1], tfit.coef[2],tfit.coef[3],tfit.coef[4]), deviance = tfit.gof)
}

hprismsplit<-function(y, wt, x, parms, continuous)
{
  n <- length(y)
  w<-kmweight.fix(y[, 1],y[, 2])
...
hprisminit<-function (y, offset, parms=0, wt)
{
  if (is.null(offset))
    offset <- 0
  sfun <- function(yval, dev, wt, ylevel, digits) {
    paste("mean=", yval[, 1], "\n",
          ", fint= ", format(signif(yval[,2], digits)), "\n",
          ", fslope=", format(signif(yval[, 3],digits)), "\n",
          ", finteraction0=", format(signif(yval[, 4],digits)), "\n",
          ", finteraction1=", format(signif(yval[, 5],digits)), "\n",
  ...
  list(y = cbind(y, parms,offset),parms=parms,numresp = 5, numy = 4,
       summary = sfun, text = tfun)
}

ulist.hprism<-list(eval=hprismeval,split=hprismsplit,init=hprisminit)
